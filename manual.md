...
配列はそのまま記述すると、配列のアドレスをスタックへプッシュします。

```
array .i
# 2520975333504
```
上の出力結果は例のひとつで、Pythonの`id`関数に依存します。
...
<length> @a
```
変数や配列の初期化を行い、そのアドレスをスタックへプッシュします。

`$a`はスタックからポップした値で初期化された変数のアドレスをスタックへプッシュします。
`@a`はスタックからポップした値の長さで初期化した配列のアドレスをスタックへプッシュします。
